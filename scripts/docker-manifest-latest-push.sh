...
DOCKER_MANIFEST="$(docker_manifest_image_name "${SERVICE_NAME}")"

echo "Pull docker images"
# shellcheck disable=SC2086,SC2068
for platform in ${SUPPORTED_PLATFORMS[@]}; do
    ${DOCKER} pull "$(docker_latest_platform_image_name "${SERVICE_NAME}" "${platform}")"
done

echo "Create and annotate manifest"
# shellcheck disable=SC2086,SC2068
for platform in ${SUPPORTED_PLATFORMS[@]}; do
    docker_latest_platform_image_name "${SERVICE_NAME}" "${platform}"
done | xargs ${DOCKER} manifest create --amend "${DOCKER_MANIFEST}"
# shellcheck disable=SC2086,SC2068
for platform in ${SUPPORTED_PLATFORMS[@]}; do
    docker_annotate_arguments "${platform}" | xargs ${DOCKER} manifest annotate "${DOCKER_MANIFEST}" "$(docker_latest_platform_image_name "${SERVICE_NAME}" "${platform}")"
done
